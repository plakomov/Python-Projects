...
# adding a status bar to the image viewer app
status = Label(root, text="Image 1 of {}".format(len(images_list)), bd=1, relief=SUNKEN, anchor=E)
...
def forward():
    ...
    if num == 5:
        ...
        num += 1
        myLabel.configure(image=images_list[num])
        status.configure(text="Image {} of {}".format(num+1, len(images_list))) # updating the status bar
    myLabel.grid(row=1, column=0, columnspan=3)
    status.grid(row=0, column=4, sticky=W + E) # gridding the status bar
...
def back():
    ...
    if num == 0:
        ...
        num -= 1
        myLabel.configure(image=images_list[num])
        status.configure(text="Image {} of {}".format(num+1, len(images_list)))
    myLabel.grid(row=1, column=0, columnspan=3)
    status.grid(row=0, column=4, sticky=W + E)
